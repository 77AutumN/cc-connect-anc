...
func decodeActionToolRequest(r io.Reader) (string, json.RawMessage, error) {
	...
	if command != "open" && command != "customer" && command != "stage" && command != "result" && command != "assignee" && command != "stage-customer-create" && command != "stage-customer-update" && command != "knowledge_search" && command != "knowledge_read" && command != "knowledge_propose" {
