...
type interactiveState struct {
	...
	// actionToken is generated by the host for this live agent process. It is
...
// SetActionHost enables one host-owned action adapter for this engine.
func (e *Engine) SetActionHost(host ActionHost) {
	e.actionMu.Lock()
	e.actionHost = host
	e.actionMu.Unlock()
}
...
func (e *Engine) getOrCreateInteractiveStateWith(sessionKey string, p Platform, replyCtx any, session *Session, sessions *SessionManager, agentOverride Agent, ccSessionKey string) *interactiveState {
	...
	if actionHostEnabled {
		...
		if err != nil {
			slog.Error("action host: failed to generate session token", "project", e.name, "error", err)
		}
	}
	...
	if inj, ok := agent.(SessionEnvInjector); ok {
		...
		if actionToken != "" {
			...
			e.actionMu.RUnlock()
			if host != nil {
				hostEnv, err := host.SessionEnv(actionToken)
				if err != nil {
					slog.Error("action host session environment failed", "project", e.name, "error", err)
					actionToken = ""
				} else {
	...
	if e.ctx.Err() != nil {
		...
		newState := &interactiveState{platform: p, replyCtx: replyCtx, agent: agent, actionToken: actionToken, eventsNeedResync: true}
...
func (e *Engine) processInteractiveEvents(state *interactiveState, session *Session, sessions *SessionManager, sessionKey string, msgID string, turnStart time.Time, stopTypingFn func(), sendDone <-chan error, replyCtx any) {
	...
	for {
		...
		case EventPermissionRequest:
			...
			e.actionMu.RUnlock()
			if host != nil {
				ref, ok := host.Match(event)
				if ok {
					actionHost, result, principal, prepared := e.prepareHostedAction(state, ref)
					if !prepared {
						break
					}
					if err := state.agentSession.RespondPermission(event.RequestID, PermissionResult{
						Behavior: "deny",
						Message:  "Action request transferred to the authenticated gateway host. End this turn without further output.",
					}); err != nil {
						sp.discard()
						e.replyWithCard(p, replyCtx, e.hostedActionFailureCard())
						e.failClosedHostedTurn(sessionKey, state, err)
						return
					}
					sp.discard()
					...
					session.AddHistory("assistant", e.i18n.T(MsgHostedActionHandoffHistory))
					sessions.Save()
					state.mu.Lock()
					state.eventsNeedResync = false
					agentSession := state.agentSession
					...
					// The host must own the whole post-marker phase. Close the
					// Claude process before exposing any approval button so an
					// Auto/sandbox-approved tool cannot run after the handoff.
					e.cleanupInteractiveState(sessionKey, state)
					if agentSession != nil && agentSession.Alive() {
						slog.Error("hosted action agent remained alive after close", "session", sessionKey)
						e.replyWithCard(p, replyCtx, e.hostedActionFailureCard())
						return
					}
...
func (e *Engine) HandleRelay(ctx context.Context, fromProject, sourceSessionKey, message string) (string, error) {
	e.actionMu.RLock()
	hostedActionsEnabled := e.actionHost != nil
	e.actionMu.RUnlock()
	if hostedActionsEnabled {
		return "", errors.New("relay is disabled for an action-host project")
	}
...
func (e *Engine) actionPrincipalForMessage(msg *Message) ActionPrincipal {
	if msg == nil {
		return ActionPrincipal{Project: e.name}
	}
